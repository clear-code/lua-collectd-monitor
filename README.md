...
$ sudo luarocks make
```
* Add settings like the following example to your collectd.conf (see [conf/collectd-remote-monitoring-example.conf](conf/collectd-remote-monitoring-example.conf) for more details)
```xml
<LoadPlugin lua>
  Globals true
</LoadPlugin>
